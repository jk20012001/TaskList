...
const tabAsSpaces = 2;
const plainDefineRE = /#define\s+(\w+)/g;
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        str.splice(1).some(s => {
            ...
            if (id) { // is identifier
                ...
                df.type = 'number';
                df.range = [0, 3];
            } else if (s === '||') { defs = []; return true; } // ignore logical OR defines all together
            return false;
        });
...
const extractParams = (() => {
    ...
    return (tokens, cache, shaderInfo, isVert, functions) => {
        const res = [];
        for (let i = 0; i < tokens.length; i++) {
    ...
    return (code, version, extensions, isVert) => {
        if (version < 310) { // keep std140 declaration, discard others
            code = code.replace(/layout\s*\((.*?)\)(\s*)(\w+)\s+(\w+)/g, (_, tokens, trailingSpaces, type, uType) => {
                ...
                if (type === 'uniform' && uType.startsWith('image')) { return _; } // keep Storage Image bindings
...
const shaderFactory = (() => {
    ...
    const createShaderInfo = () => ({ blocks: [], samplerTextures: [], samplers: [], textures: [], buffers: [], images: [],
        subpassInputs: [], attributes: [], varyings: [], fragColors: [] });
    const compile = (name, isVert, outDefines = [], shaderInfo = createShaderInfo(), chunks = globalChunks, deprecations = globalDeprecations) => {
        const out = {};
        shaderName = name;
        const cache = { lines: [], extensions: {} };
        const { code, record, functions } = preprocess(name, chunks, deprecations, isVert);
        const tokens = shaderTokens = tokenizer(code, tokenizerOpt);
        cache.existingDefines = extractMacroDefinitions(code);
        extractDefines(tokens, outDefines, cache);
        const blockInfo = extractParams(tokens, cache, shaderInfo, isVert, functions);
    ...
    const build = (vertName, fragName, chunks = globalChunks, deprecations = globalDeprecations) => {
        let defines = [], shaderInfo = createShaderInfo();
        const src = {};
        src.vert = compile(vertName, true, defines, shaderInfo, chunks, deprecations);
        src.frag = compile(fragName, false, defines, shaderInfo, chunks, deprecations);
        ...
        shaderInfo.blocks.forEach((b) => (delete b.tags, b.members.forEach((v) => (delete v.typename, delete v.precision, delete v.isArray))));
        shaderInfo.samplerTextures.forEach((v) => (delete v.tags, delete v.typename, delete v.precision, delete v.isArray));
        shaderInfo.buffers.forEach((v) => (delete v.tags, delete v.typename, delete v.precision, delete v.isArray, delete v.members));
        shaderInfo.images.forEach((v) => (delete v.tags, delete v.typename, delete v.precision, delete v.isArray));
        shaderInfo.textures.forEach((v) => (delete v.tags, delete v.typename, delete v.precision, delete v.isArray));
        shaderInfo.samplers.forEach((v) => (delete v.tags, delete v.typename, delete v.precision, delete v.isArray));
        shaderInfo.subpassInputs.forEach((v) => (delete v.tags, delete v.typename, delete v.precision, delete v.isArray, delete v.type));
        // assign bindings
        let bindingIdx = 0;
        ...
        src.vert.glsl4 = stripToSpecificVersion(decorateBindings(src.vert.glsl4, shaderInfo, src.vert.blockInfo), 460, src.vert.extensions, true);
        src.frag.glsl4 = stripToSpecificVersion(decorateBindings(src.frag.glsl4, shaderInfo, src.frag.blockInfo), 460, src.frag.extensions, false);
...
const buildEffect = (name, content) => {
    ...
    for (const jsonTech of effect.techniques) {
        for (const pass of jsonTech.passes) {
            const vert = pass.vert, frag = pass.frag;
            delete pass.vert; delete pass.frag;
            const name = pass.program = `${effectName}|${vert}|${frag}`;
